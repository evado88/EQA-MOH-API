...
class ProviderDB(Base):
    ...
    # a provider is identified by its name
    ...
    #relationships
    user = relationship("UserDB", back_populates="provider", lazy='raise')
    status = relationship("StatusDB", back_populates="provider", lazy='raise')
    stage = relationship("StageDB", back_populates="provider", lazy='raise')

    #links
    scheme = relationship("SchemeDB", back_populates="provider", lazy='raise')
...
class ProviderWithDetail(Provider):
    stage: Stage
    status: Status
    user: User
    

class ParamProviderEdit(BaseModel):
    provider: Optional[ProviderWithDetail] = None
