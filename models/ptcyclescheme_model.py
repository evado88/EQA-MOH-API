...
# ---------- SQLAlchemy Models ----------
class PTCycleSchemeDB(Base):
    __tablename__ = "pt_cycle_schemes"
    ...
    #relationships
    user = relationship("UserDB", back_populates="ptcyclescheme", lazy='raise')
    status = relationship("StatusDB", back_populates="ptcyclescheme", lazy='raise')
    stage = relationship("StageDB", back_populates="ptcyclescheme", lazy='raise')

    #links

# ---------- Pydantic Schemas ----------
class PTCycleScheme(BaseModel):
    # id
...
class PTCycleSchemeWithDetail(PTCycleScheme):
    stage: Stage
    status: Status
    user: User
    

class ParamPTCycleSchemeEdit(BaseModel):
    ptcyclescheme: Optional[PTCycleSchemeWithDetail] = None
