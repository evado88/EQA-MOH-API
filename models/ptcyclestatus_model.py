...
# ---------- SQLAlchemy Models ----------
class PTCycleStatusDB(Base):
    __tablename__ = "pt_cycle_statuses"
    # a cycle status is identified by its name
    __table_args__ = (UniqueConstraint("name", name="uq_pt_cycle_statuses_name"),)
    ...
    #relationships
    user = relationship("UserDB", back_populates="ptcyclestatus", lazy='raise')
    status = relationship("StatusDB", back_populates="ptcyclestatus", lazy='raise')
    stage = relationship("StageDB", back_populates="ptcyclestatus", lazy='raise')

    #links
    ptcycle = relationship("PTCycleDB", back_populates="ptcyclestatus", lazy='raise')

# ---------- Pydantic Schemas ----------
class PTCycleStatus(BaseModel):
    # id
...
class PTCycleStatusWithDetail(PTCycleStatus):
    stage: Stage
    status: Status
    user: User
    

class ParamPTCycleStatusEdit(BaseModel):
    ptcyclestatus: Optional[PTCycleStatusWithDetail] = None
