...
# ---------- SQLAlchemy Models ----------
class LabTypeDB(Base):
    __tablename__ = "lab_types"
    # a lab type is identified by its name
    __table_args__ = (UniqueConstraint("name", name="uq_lab_types_name"),)
    ...
    #relationships
    user = relationship("UserDB", back_populates="labtype", lazy='raise')
    status = relationship("StatusDB", back_populates="labtype", lazy='raise')
    stage = relationship("StageDB", back_populates="labtype", lazy='raise')

    #links
    laboratory = relationship("LaboratoryDB", back_populates="labtype", lazy='raise')

# ---------- Pydantic Schemas ----------
class LabType(BaseModel):
    # id
...
class LabTypeWithDetail(LabType):
    stage: Stage
    status: Status
    user: User
    

class ParamLabTypeEdit(BaseModel):
    labtype: Optional[LabTypeWithDetail] = None
